 ...
 *    microcontroller?
 *   255 is the max the benfits are it's faster uses less storage drawback are only gose up to 255
 * 
 ...
 *   const is fast as the compiler dose'nt need to check if it get updated
 * 
 ...
 *   the prgame adds one to the count when the loop is run about ever 12ms 
 * 
 ...
 *    if the led stay turns off the the count has gone past 255 and looped back
 * 
 ...
 * the max count it can reah now is 255 and the led will be on and won't turn off 
 *
